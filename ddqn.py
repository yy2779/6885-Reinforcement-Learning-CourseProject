...
class DoubleDQN:
    ...
    def _build_net(self, scale, fc_map):
        ...
        with tf.variable_scope('train'):
            ...
            learning_rate = tf.train.exponential_decay(self.lr, global_step, 1000, 0.99, staircase=True)
    ...
    def learn(self):
        ...
        q_next, q_eval4next = self.sess.run([self.q_next, self.q_eval], feed_dict={self.s: samples[:, -self.n_features:],
                                                                              self.s_: samples[:, -self.n_features:]})
        q_eval = self.sess.run(self.q_eval, feed_dict={self.s: samples[:, :self.n_features]})

        q_target = q_eval.copy()

        eval_act_index = samples[:, self.n_features].astype(int).reshape(self.batch_size, 1)
        reward = samples[:, self.n_features+1].reshape(self.batch_size, 1)
        max_act4next = np.argmax(q_eval4next, axis=1).reshape(self.batch_size, 1)
        selected_q_next = np.zeros([self.batch_size, 1])
        for i in range(self.batch_size):
            selected_q_next[i] = q_next[i, max_act4next[i]]
